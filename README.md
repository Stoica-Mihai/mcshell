...
| `Bar/` | Status bar — three parallelogram segments, each with one `AnimatedPopup` dropdown that swaps content via `activePanel`. Left: launcher + workspaces, active window; dropdown hosts keybind hints. Center: clock + calendar/clock-settings, weather + forecast/edit; dropdown hosts calendar / weather / clock-settings. Right: media (MPRIS), system tray (collapsed ring + expanded grid), WiFi/BT/volume-waveform/battery/notifications/system-monitor capsule; dropdown hosts volume / notifications / media / sysinfo / sysinfo-settings / trayicons. Also `Recording` (wf-recorder driver), `SysInfoPanel` (CPU/RAM/temp/disk/network/GPU dashboard) |
