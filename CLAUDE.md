...
| `WindowSwitcher/` | `WindowSwitcher` — fullscreen overlay with carousel cards showing open windows (icon via `DesktopEntries.heuristicLookup` + title). Navigate with arrows, Enter to focus via niri IPC. Alt+Tab behavior: niri keybind opens/cycles, Alt release activates selected window. Search bar wrapped in `ParallelogramCard` with scaled skew. Wrap-around navigation. |
