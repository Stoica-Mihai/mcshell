...
| `Bar/` | `StatusBar` — three parallelogram segments (left/center/right) with Shape backgrounds and static gradient border (`BarBorder` — solid or gradient style via `UserSettings.barBorderStyle`). Fixed width for left/right segments (`Theme.barSideWidth`). Left segment: two zones (launcher+workspaces locked left, active window right). Center segment: `Clock` with shared dropdown for `CalendarPopup`. Right segment: two zones (media locked left, systray+capsule locked right) with shared dropdown (`sharedDropdown`) for volume, notifications, media player, and tray context menus. Both dropdowns use `AnimatedPopup`'s panel API (`togglePanel`/`openPanel`/`closePanel`). Components: `Workspaces`, `ActiveWindow`, `Clock`, `CalendarPopup`, `Media` (MPRIS — bar controls only, popup content in shared dropdown), `Network`, `Volume`, `Battery`, `SysTray`, `CapsuleItem`, `MenuItem`. |
