...
| `Core/` | Non-visual shared components — `SafeProcess` (process wrapper with error logging), `ShellActions` singleton (session actions: lock, logout, reboot, shutdown, wallpaper — callable from any component), `Brightness` singleton (screen brightness via `brightnessctl`, event-driven via `FileView` sysfs watcher), `HolidayService` singleton (fetches public holidays from Nager.Date keyed by `UserSettings.weatherCountryCode`, caches per year, lazy-fetches on first binding access, clears cache on country change). |
| `Bar/` | `StatusBar` — three parallelogram segments (left/center/right) with Shape backgrounds and static gradient border (`BarBorder` — solid or gradient style via `UserSettings.barBorderStyle`). Fixed width for left/right segments (`Theme.barSideWidth`). Left segment: two zones (launcher+workspaces locked left, active window right). Center segment: `Clock` (left-click opens `CalendarWindow`, right-click opens `ClockSettingsWindow`), separator, `Weather` (left-click opens `WeatherWindow`, right-click opens weather in edit mode). Right segment: two zones (media locked left, systray+capsule locked right) with shared dropdown (`sharedDropdown`) for volume, notifications, media player, and tray context menus. Calendar/Weather/ClockSettings windows use the shared `BarPopupWindow` (layer shell with keyboard focus); the right-segment dropdown uses `AnimatedPopup`'s panel API (`togglePanel`/`openPanel`/`closePanel`). `CalendarPopup` reads public holidays from `HolidayService` (keyed by `UserSettings.weatherCountryCode`): holiday days get a dot marker, and the viewed month's holidays are listed below the grid. Components: `Workspaces`, `ActiveWindow`, `Clock`, `CalendarPopup`/`CalendarWindow`, `Weather`/`WeatherPopup`/`WeatherWindow`, `ClockSettingsPopup`/`ClockSettingsWindow`, `BarPopupWindow`, `Media` (MPRIS — bar controls only, popup content in shared dropdown), `Network`, `Volume`, `Battery`, `SysTray`, `CapsuleItem`, `MenuItem`. |
...
| `Widgets/` | Shared UI components — `AnimatedPopup` (panel API: `activePanel`, `anchorSection`, `togglePanel`/`openPanel`/`closePanel`, `autoPosition`), `AnimatedBorder` (pluggable card border animations: midpoint, clockwise, corners, fade), `IconButton`, `SliderTrack`, `ControlSlider`, `CyclePicker`, `ParallelogramCard` (overridable `_skew`), `StyledTextField` (search fields), `MediaControls` (MPRIS transport buttons), `ActiveUnderline` (accent indicator), `HoverText`, `OptImage`, `Separator`, `SmoothWheelHandler`, `ThemedScrollBar`, `TriToggle`. |
